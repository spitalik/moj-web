...
- **`sw.js`** — service worker. Precaches an `ASSETS` list (must include every game's `index.html`) under a versioned cache name (`arkadium-v1`). Fetch strategy: **network-first for `.html`/`.js`** (so new games/strings appear without a manual reload), **cache-first for other static assets** (icons). On install it `skipWaiting()`s; the launcher reloads once on `controllerchange` to pick up updates.
...
2. **`sw.js`** — add `'./games/<id>/index.html'` to the `ASSETS` array **and bump the `CACHE` version** (e.g. `arkadium-v1` → `arkadium-v2`). Bumping the version is what forces clients to re-cache; forgetting it means users keep the stale asset list.
